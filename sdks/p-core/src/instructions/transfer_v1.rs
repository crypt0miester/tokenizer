...
impl TransferV1<'_> {
    ...
    pub fn invoke_signed(&self, signers: &[Signer]) -> ProgramResult {
        let account_metas = [
            ...
            InstructionAccount::readonly(self.new_owner.address()),
            InstructionAccount::readonly(self.system_program.address()),
            InstructionAccount::readonly(self.log_wrapper.address()),
        ];

        let mut instruction_data = [UNINIT_BYTE; 2];
        write_bytes(&mut instruction_data[0..1], &[14]); // TransferV1 discriminator
        ...
        invoke_signed(
            &instruction,
            &[self.asset, self.collection, self.payer, self.authority, self.new_owner, self.system_program, self.log_wrapper],
            signers,
        )
    }
}
